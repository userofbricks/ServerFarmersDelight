...
public class RottenTomatoItem extends Item implements ProjectileItem
{
	...
	public ActionResult use(World level, PlayerEntity player, Hand hand) {
		...
		if (!level.isClient) {
			RottenTomatoEntity projectile = new RottenTomatoEntity(level, player, heldStack.copyWithCount(1));
	...
	public ProjectileEntity createEntity(World level, Position position, ItemStack itemStack, Direction direction) {
		RottenTomatoEntity rottenTomato = new RottenTomatoEntity(level, position.getX(), position.getY(), position.getZ(), itemStack);
